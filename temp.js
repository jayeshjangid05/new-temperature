...
function GetData() {
  ...
  while (true) {
    if (date2.getTime() > date1.getTime()) {
      let vardate = new Date(date1.setMinutes(date1.getMinutes() + diff));;
      Timearr.push(vardate);
    } else {
      break;
    }
...
function PrintSerialNumner(Timearr){
      for (let i = 1; i <=Timearr.length; i++) {//by jj add equal sign
...
function PrintDateTime(Timearr){
  ...
  for (let a of Timearr) {
    if (datecount < Timearr.length) {//by jj remove timearr.length-1
